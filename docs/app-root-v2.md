# LuminaCode AppRoot v2

AppRoot v2 is the shared storage contract used by the Go backend, TypeScript
...
unsupported version. Fresh installation and v1 conversion initialize it only
...
uses a migration lock, verifies copied content, imports legacy Markdown memory,
and writes `state/migrations/v2-report.json`. `layout.json` is committed only
...
to `data/legacy/v1/projects` and their trust remains inactive until explicitly
bound.
