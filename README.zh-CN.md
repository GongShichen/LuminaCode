...
- 持久化 message cursor 会先提交可见消息，再运行语义 enrichment。每条消息
  保存为不可变 evidence span、重叠 chunk，以及按句子、列表项和代码块切分的
  evidence atom。
- Evidence atom 目标大小为 96 tokens，保留 message ID、Session ID、角色、
  rune offset、时间、content hash 和 scope；原始消息与 chunk 可用于重建索引。
- 语义 enrichment 生成 memory、fact、entity、relation、canonical event 和
  core block。候选只引用 source message ID，并通过可恢复后台任务执行。
- SQLite FTS5 为 memory、Session、chunk 和 atom 建立全文索引；本地归一化
  embedding 与来源记录保存在同一数据库，content hash 保证写入和 backfill 幂等。

### 检索与证据组装

- 每次查询固定运行 BM25、本地向量、实体、时间、Session 和图六个通道。查询
  扩展只能增加同义表达、实体、时间提示、关系词和 provenance 提示，不能选择
  通道、scope、权重或 memory type。
- BM25、向量、实体、时间和 Session 通道并行执行；图通道扩展融合后的候选，
  相关 Session 还会执行 SQL 层约束的 atom 检索。
- 通道结果归入 lexical、semantic、entity、time、Session 和 relation 六类独立
  signal family，RRF 对每个 family 的最佳贡献计票一次。
- Evidence Ledger 从原始查询与合法扩展构建通用 facet，依据融合相关性、未覆盖
  facet 增益、epistemic provenance、来源贡献、实体/时间一致性和 token 成本选择
  atom，并可执行一轮全通道 residual coverage sweep。
- Evidence packet 先装入直接命中的 atom；同一消息中的 atom 按原始位置合并，
  再补充受限的相邻上下文。Core block 使用独立预算，packet 保留来源 ID、角色、
  时间、有效区间、provenance 和 timeline。
- 本地 ONNX 推理使用 backend 级共享 scheduler，分别维护 query/document 队列，
  支持 micro-batch、内容缓存、查询缓存，并从获得执行槽后计算执行 timeout。

### 时间、来源与隔离

- 每个用户 Turn 使用同一个参考时间，贯穿查询扩展、时间索引、timeline 和隐藏
  回答上下文。
- Fact 同时保存有效时间 `valid_from / valid_until` 与观测时间
  `observed_at / invalidated_at`；更新保留旧版本，并建立 `supersedes` 和
  `contradicts` 关系。
- Evidence 使用 `reported`、`observed`、`derived`、`suggested`、
  `hypothetical`、`questioned` 标记 epistemic status。Canonical entity/event
  保留全部来源 atom 及其 scope。
- user、project、Team、agent type 和 Team agent scope 在每个检索通道及图遍历中
  执行 ACL。召回证据是临时隐藏上下文，不进入可见 transcript。

### 生命周期与治理

- 事实有效期与存储保留期相互独立。Active memory 根据访问和再次确认信号在
  `hot / warm / cold` 之间变化。
- 生命周期价值由 importance、confidence、访问时效/频率、reinforcement、
  provenance strength 和 dependency strength 组成。到期、未 pin 且低价值的记录
  可在宽限期后归档。
- 后台维护不执行物理删除。Pin、有效依赖、core block 和未解决冲突链受到保护；
  archived 记录可以恢复并重新建立索引。
...
| `memory_atom_target_tokens` / `memory_atom_max_tokens` | 96 / 160 | evidence atom 大小 |
| `memory_atom_max_selected` | 32 | 安全上限，正常数量由 token budget 决定 |
| `memory_coverage_max_facets` | 8 | Evidence Ledger 跟踪的通用查询 facet 数 |
| `memory_coverage_completion_rounds` | 1 | 补充覆盖时执行全通道检索的轮数 |
| `memory_adjacent_chunk_window` | 1 | 直接证据之后考虑的相邻 atom 窗口 |
| `memory_embedding_batch_size` / `memory_embedding_batch_wait_ms` | 32 / 20 | 本地 embedding 共享 micro-batch |
| `memory_retrieval_cache_ttl_seconds` | 300 | 按 scope 隔离的召回缓存时间 |
| `memory_query_expansion_timeout_seconds` | 4 | 通用查询扩展的最大等待时间 |
...
五个 `memory_coverage_*_weight` 和三个 `memory_evidence_*_budget_ratio` 参数各自
必须合计为 `1`。废弃的 `memory_mmr_*` 与 `memory_recall_max_items` 仍可读取以
兼容旧配置，但不再控制证据选择。配置会在下一个 Turn 热加载；`make install`
只补充新默认字段，不会覆盖用户已有值。
...
Lumina 在 500 题 oracle 数据集上的成绩为 **83.2%（416/500）**。答案通过
`https://api.deepseek.com` 使用 LongMemEval 官方判分 prompt 和
`deepseek-v4-pro` 完成评估。本次运行通过生产写入与检索链路进行完整黑盒测试，
但不是官方 GPT-4o leaderboard 成绩。
...
| 平均记忆上下文 | 1,277 tokens（memory token ratio 21.10%） |
| 平均检索耗时 | 3.52 秒 |
...
| **LuminaCode** | **83.2%** | DeepSeek Judge，复用官方 prompt；完整 500 题 |
